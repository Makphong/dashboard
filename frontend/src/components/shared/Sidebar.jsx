...
export const Sidebar = React.memo(({ isMobileOpen, setMobileOpen, isCollapsed, toggleCollapse, activeView, setActiveView }) => (
  <aside className={`scroll-clarity-layer fixed inset-y-0 left-0 z-[150] bg-white border-r border-[#d7e8f6] transition-[transform,width] duration-300 ease-out flex flex-col
    ...
    <div className={`h-20 shrink-0 flex items-center border-b border-[#d7e8f6] overflow-hidden transition-[padding,justify-content] duration-300 ${isCollapsed ? 'justify-center px-3 lg:px-0' : 'justify-start px-6'}`}>
      <div className="flex items-center gap-2 min-w-max">
        ...
        <span className={`text-2xl font-extrabold tracking-tight text-[#17335f] ${labelClass(isCollapsed)}`}>
          Audit Log
        </span>
      </div>
