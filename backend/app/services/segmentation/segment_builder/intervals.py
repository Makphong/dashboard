...
def _overlap_review_actor_segments(interval: dict) -> list[dict]:
    ...
    if interval["exit_to"] == COMPLETED_STATE:
        overlap_segment_type = _completion_segment_type(
            overlap_edit_count,
...
def _in_review_segments(interval: dict, all_events: list[dict]) -> list[dict]:
    overlap_segments = _overlap_review_actor_segments(interval)
    if overlap_segments:
